...
*/
  ...
  */
  ...
  sprintf (outfile,"%s_singlex_%0.1fsigma_noise%0.3f.root",outfilx,Nsigma,pedwidth);
#else
  //sprintf (outfile,"%s_multix.root",outfilx);
  sprintf (outfile,"%s_multix_%0.1fsigma_noise%0.3f.root",outfilx,Nsigma,pedwidth);
#endif
  ...
  while(!(file_db.eof())) {
    ...
    file_db >> datafile>>nentrymx >> genE;
    ...
    */
	...
	for(int idigi=0; idigi<digi.size(); idigi++){
	  ...
	  if(!fill2D && digi.size() > 0 && digi.size()!=digihitpos.size() && allclusters_EMalgo.size()!=cluster_digihitposMod.size() && icl==0) {
	...
	for(int idigi=0; idigi<hitsFrac.size(); idigi++){
      ...
      int ips[nvar]={0};
      int ipsall = 0;
      if (allphoton.size()>0 && allphoton[0].ppho.mag() >5.0) { 
...
void ecal_clustering(vector<Hep3Vector> digienr, double seedthres, double thresh, vector<phoparam>& recopho, vector< vector < Hep3Vector > >& cluster_digihitposMod) {
  ...
  */
  ...
  for (int ij=0; ij<digienr.size(); ij++) {
    ...
    //digihitposMod.push_back(digienr[ij]); ///SJ
    ij--;
    int ncount=1;
...
/// expectation maximization method
  ...
  */
  ...
  //cout<<"SJ!!! number of initial seeds before removal of neighbours "<<seeds.size()<<endl;
  ...
  for(int iclus=0; iclus<seeds.size(); iclus++){
    ...
    for(int jclus=iclus+1; jclus<seeds.size(); jclus++){
      ...
      if( angle*radtodeg<1.5 ) ///sqrt(2) --> neighboring, touching the sides. Extent of ECAL xtal in degree is 1 degree
